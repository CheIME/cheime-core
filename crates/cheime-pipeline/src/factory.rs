use crate::{
    CodeSegment, ComposablePipeline, Filter, Processor, Ranker, Segmentor, Translator,
};
use cheime_config::schema::{
    EngineConfig, FilterConfig, ProcessorConfig, SchemaConfig, SegmentorConfig,
};
...
impl PipelineFactory {
    pub fn build(
        ...
        Ok(ComposablePipeline::new(
            Self::build_processor(&config.engine)?,
            Self::build_segmentor(&config.engine)?,
            Self::build_translators(&config.engine, user_store)?,
    ...
    fn build_translators(
        ...
        if let Some(s) = user_store {
            out.push(Box::new(UserDictTranslator::new(s)));
        }
        if out.is_empty() {
    ...
    fn build_filters(e: &EngineConfig) -> Result<Vec<Box<dyn Filter>>, BuildError> {
        ...
        for f in &e.filters {
            if matches!(f, FilterConfig::Uniquifier) {
...
impl std::fmt::Display for BuildError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BuildError::UnsupportedComponent { component_type, pipeline_stage } =>
                write!(f, "unsupported '{component_type}' in {pipeline_stage}"),
            BuildError::MissingDictionary { name } => write!(f, "dictionary '{name}' not found"),
        }
    }
}
...
mod tests {
    ...
    fn user_word_first() {
        ...
        s.apply(cheime_user_data::UserEvent::learn_word("t", "qp", "你", "ni"));
